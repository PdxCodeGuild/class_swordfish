# Merritt's Notes / Solutions for Lab 8 Contacts CSV
...
keys = list_of_lists[0] #pulls out list index 0, which is the headers
print(keys)
# contacts = []
...
#     #     # row_dict['name'] = row[0]
#     #     # row_dict[keys[1]] = row[1]
#     #     # row_dict['favorite fruit'] = row[2]
...
print(contacts)
# contacts = [dict(zip(keys, values)) for values in [line.split(',') for line in data_csv.split('\n')][1::]]

def create_contact(data, keys):
    # new_contact = {}
    # for key in keys:
    #     new_contact[key] = input(f"What is your contact's {key}? ")
    # data.append(new_contact)
    data.append({key: input(f"What is your contact's {key}? ") for key in keys})

def retrieve_contact(data, keys):
    ...
    # data_results = []
    # for contact in data:
    #     if contact[key_input] == value_input:
    #         data_results.append(contact)

    # data_results = [contact for contact in data if contact[key_input] == value_input]

    # def filter_function(contact):
    #     return contact[key_input] == value_input
    # data_results = list(filter(filter_function, data))

    data_results = list(filter(lambda contact: contact[key_input] == value_input, data))
...
def update_contact(data, keys):
    ...
    data_results = retrieve_contact(data, keys)
...
def delete_contact(data, keys):
    # result = retrieve_contact(data, keys)
    ...
    index_to_remove = int(input(f"Which contact would you like to delete? (1-{len(data_results)}) ")) - 1
    confirmation = input(f"Are you sure you want to delete {data_results[index_to_remove]['name']}? ")
    if confirmation.lower() in ['y', 'yes', 'true', 'go', 'do it']:
        data.remove(data_results[index_to_remove])


while True:
    user_input = input("What would you like to do? (c)reate, (r)ead, (u)pdate, (d)elete, (q)uit ")
    if user_input.lower() in ['q', 'quit', 'exit', 'stop', 'abort']:
        break
    elif user_input == 'c':
        create_contact(contacts, keys)
    elif user_input == 'r':
        retrieve_contact(contacts, keys)
    elif user_input == 'u':

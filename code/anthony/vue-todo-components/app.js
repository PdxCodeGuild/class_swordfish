  ...
  <div :class="className">
    ...
    <ul>
      <li v-for="todo in todos">
        {{todo.text}}
        <button v-on:click="emitToggle(todo)">&check;</button>
        <button v-on:click="emitRemove(todo)">&cross;</button>
      </li>
    </ul>
  </div>
  `,
  props: ["className", "title", "todos"],
  methods: {
    emitToggle: function (todo) {
      this.$emit('toggle', todo)
    },
    emitRemove: function (todo) {

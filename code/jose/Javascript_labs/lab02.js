...
let x = document.getElementById(parseInt(('number')))
...
numberspelled.addEventListener("click", function(){
    console.log(x)
    if (x.value in random_numbers) {
        numberspelled.Innertext = random_numbers[x]  
    } 
    if (x.value in tens_under20){
        numberspelled.Innertext = tens_under20[x]
    } else if (length_of_array >= 2 && x >= 20 && x <100){
        let first_word = ones[intArr[0]]
        let second_word = tens_digitwritten[intArr[1]]
        numberspelled.Innertext = second_word+first_word
    }  else if (length_of_array >= 3 && x >= 100){
        let first_word = ones[intArr[0]]
        let second_word = tens_digitwritten[intArr[1]]
        let third_word = ones[intArr[2]]
        numberspelled.Innertext = first_word + "hundred "+ second_word+" "+ third_word
    }})

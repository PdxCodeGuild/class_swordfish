...
user_input = input('Would you like to play a trivia game?: ').capitalize()
...
while user_input == 'Yes':
    ...
    difficulty = input('Please select your level of difficulty(Easy/Medium/Hard) or "quit" to exit: ').lower()
    if difficulty == 'quit':
        break
    ...
    response = requests.get('https://opentdb.com/api.php?amount=10&category=' + category_number + '&difficulty=' + difficulty, headers = {'Authorization': 'Token token="855df50978dc9afd6bf86579913c9f8b"'})
    results = response.json()
    # print(results)
    computer_score = ()
    ...
    for i, question in enumerate(questions_list):
        ...
        new_question = question['question'].replace('&#039;', "'")             #replacing the value of &#039 to the actual string representation of '
        question['question'].replace('&quot;', '"')                                  
        print(new_question)
        true_answer = [question['correct_answer']]
        false_answers = question['incorrect_answers']
        # random_index = random.randint(0,3)
        shuffled_answers = false_answers + true_answer
        time.sleep(2)

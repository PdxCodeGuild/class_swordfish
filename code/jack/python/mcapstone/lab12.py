...
url = 'https://elavegan.com/easy-lentil-stew-mashed-potatoes-vegan/'
...
for x in range(len(file_title)):
    print(x)
    if file_title[x] not in string.ascii_letters or file_title[x] != '_':
        file_title = file_title[:x - 1]
print(file_title)

# with open('storage.txt', 'w') as f: # save all text to file
#     f.write(all_text)
...
ingredient_keywords = ['ingredients', 'cup', 'tbsp', 'tablespoon', 'tsp', 'teaspoon'] # list of keywords commonly found in ingredient list
